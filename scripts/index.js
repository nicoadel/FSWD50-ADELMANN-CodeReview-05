$( document ).ready(function() {


const itemsData = [{
        "name": "The Shawshank Redemption",
        "art": "Drama",
        "likes": "9",
        "description" :"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed est velit, tempus vel justo ut, euismod luctus lacus. Phasellus consequat quam metus",
        "photo": "https://images-na.ssl-images-amazon.com/images/I/512G9J05RJL._SY445_.jpg"
    },
    {
        "name": "The Godfather",
        "art": "Adventure",
        "likes": "9",
        "description" :" at fringilla mauris placerat non. Curabitur l",
        "photo": "https://image-ticketfly.imgix.net/00/02/76/61/69-og.jpg?w=500&h=750"
    },
    {
        "name": "Star Wars",
        "art": "Action",
        "likes": "3",
        "description" :"Etiam eleifend, nibh sed tempus aliquam, mi turpis gravida augue, nec dictum tortor lorem quis nisi. Aliquam scelerisque, felis pharetra dictum pharetra,",
        "photo": "https://m.media-amazon.com/images/M/MV5BOTAzODEzNDAzMl5BMl5BanBnXkFtZTgwMDU1MTgzNzE@._V1_.jpg"
    },
    {
        "name": "Random Movie Name",
        "art": "Drama",
        "likes": "4",
        "description" :". Nulla consequat lacinia arcu a malesuada",
        "photo": "https://image-ticketfly.imgix.net/00/02/76/61/69-og.jpg?w=500&h=750"
    },
    {
        "name": "An Adventure Movie",
        "art": "Adventure",
        "likes": "9",
        "description" :"eros dui. Nullam sapien eros, ullamcorper quis lectus ac, blandit rh",
        "photo": "data:image/jpeg;base64,"
    },
    {
        "name": "ted",
        "art": "Drama",
        "likes": "6",
        "description" :" molestie. Quisque tincidunt magna tortor, a dapibus nisi aliquam id. Vivamus ",
        "photo": "http://gdj.graphicdesignjunction.com/wp-content/uploads/2012/10/movie+posters+45.jpg"
    },
    {
        "name": "Scary",
        "art": "Action",
        "likes": "8",
        "description" :"Integer hendrerit lacus id ex cursus, vitae convallis ligula tempus. Phasellus et dui mauris. ",
        "photo": "http://waytooindie.com/wp-content/uploads/2015/09/jaws-movie-cover.jpg"
    },
    {
        "name": "Ladykiller",
        "art": "Drama",
        "likes": "7",
        "description" :" molestie. Quisque tincidunt magna tortor, a dapibus nisi aliquam id. Vivamus ",
        "photo": "https://mir-s3-cdn-cf.behance.net/project_modules/disp/33c6d427397067.56364780ebd6e.jpg"
    },
    {
        "name": "Magicstuff",
        "art": "Action",
        "likes": "2",
        "description" :"ces vestibulum, odio purus varius eros, eget rhoncus urna te",   
        "photo": "http://www.coverwhiz.com/content/Harry-Potter-And-The-Chamber-Of-Secrets.jpg"
    },
    {
        "name": "Magicstuff",
        "art": "Action",
        "likes": "8",
        "description" :"aretra, mi arcu fermentum velit, ac faucibus ante odio vitae libero. Vivamus qui",
        "photo": "http://www.coverwhiz.com/content/Harry-Potter-And-The-Chamber-Of-Secrets.jpg"
    }

];

  //---------------Displaying JSON Items in item_container--------------//

    document.getElementById("item_container").innerHTML =
        `${itemsData.map(itemShoppingCart).join('')}`


   function itemShoppingCart(item) {
        return `
                <div class="col-lg-4 col-md-6 col-sm-6 col-xsm-12 ">
                <div class="block_element rounded">
                    <img class="p-2 rounded" src="${item.photo}" alt="">
                    <div class="movie_description">
                        <h4 class="pt-3 font-weight-bold">${item.name}</h2>
                        <h5>${item.art}</h3> 
                        <h5>${item.description}</h4>
                        <p><button class="like btn btn-success mr-3">Like</button><span class="font-weight-bold font-italic">${item.likes}</span><p>
                    </div>
                </div>
            </div>
                `
    }
//-----------------Like Counter--------------------//

$(".like").click(like);

function like() {
    var temp = Number($(this).next().html());
    temp += 1;
    console.log(temp);
    $(this).next().html(temp);
}


//------------------SORT ALGORITHM----------------//

var action = $.grep(itemsData, function(element){
          return element.art == "Action";
    });
var adventure = $.grep(itemsData, function(element){
          return element.art == "Adventure";
    });
var drama = $.grep(itemsData, function(element){
          return element.art == "Drama";
    });

function allSort(item){
        document.getElementById("item_container").innerHTML =
        `${itemsData.map(itemShoppingCart).join('')}`
        $(".like").click(like);

}
function actionSort(item){
        document.getElementById("item_container").innerHTML =
        `${action.map(itemShoppingCart).join('')}`
        $(".like").click(like);

}
function adventureSort(item){
        document.getElementById("item_container").innerHTML =
        `${adventure.map(itemShoppingCart).join('')}`
        $(".like").click(like);

}
function dramaSort(item){
        document.getElementById("item_container").innerHTML =
        `${drama.map(itemShoppingCart).join('')}`
        $(".like").click(like);

}
document.getElementById("btn_all").addEventListener("click",allSort);
document.getElementById("btn_action").addEventListener("click",actionSort);
document.getElementById("btn_adventure").addEventListener("click",adventureSort);
document.getElementById("btn_drama").addEventListener("click", dramaSort);

}); 